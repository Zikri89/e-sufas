...
class Faskes_assesment extends CI_Controller {
    ...
    {
        $data = array(
            'main_title'    =>  'Kesehatan Ibu dan Anak',
    ...
    {
        $result = $this->assesment_model->side_server_json_assesment_kia($param);
    }

    function side_server_json_hasil_assesment_kia()
    {
        error_reporting(0);
        ...
        $result = $this->assesment_model->side_server_json_hasil_assesment_kia($semester,$faskes,$converttgl1,$converttgl2);
    }
    ...
    public function my_assesment() {
        ...
        foreach ($ids as $id => $asses) {
            ...
            $this->db->insert('kia_assesment',$data);

            $count_kialist      =  $this->db->count_all_results("kia_list")-23;//total item apn di kurangi 23 item title kia 
            $count_kiaassesment = $this->db->where('kode_faskes',$this->session->userdata('assesment_faskes'))->count_all_results("kia_assesment");
            $persen = ceil(($count_kiaassesment/$count_kialist) * 100);
